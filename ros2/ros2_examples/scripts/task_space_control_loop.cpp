...
public:
  explicit TaskSpaceControlLoop(
      const std::string& node_name, const std::chrono::nanoseconds& dt, double distance_tolerance
  ) :
  ...
  void init() {
    ...
    this->dynamical_system_->set_parameter_value("attractor", this->target_);
    // set a starting pose
  ...
  void run() {
    ...
    RCLCPP_INFO(this->get_logger(), "Attractor reached, shutting down ROS");
    rclcpp::shutdown();
  }
};
}
